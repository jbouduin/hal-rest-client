...
describe('JSON Parser: links', () => {
  ...
  const testHalResource = (resource: HalResource) => {
    ...
    expect(link1.uri.href).toBe<string>('/link1')
    ...
    expect(link1b.uri.href).toBe<string>('/link1')
  ...
  const testToHalResourceModel = (model: ToHalResourceModel) => {
    ...
    expect(model.link1.uri.href).toBe<string>('/link1');
  ...
  test('Halmodel linking to HalResource - mixed links', () => {
    ...
    expect(model.link3.uri.href).toBe<string>('/link3');
  });
  ...
  test('extended link properties', () => {
    ...
    expect(link2.uri.href).toBe<string>('/link2');
    ...
    expect(link3.uri.href).toBe<string>('/link3');
  ...
  test('link with href: null', () => {
    ...
    const resource = parser.objectToHalResource(client, json, '', HalResource);
    expect((resource.getLink<IHalResource>('link1')).uri.href).toBeNull();
    expect((resource.getProp<IHalResource>('link1')).uri.href).toBeNull();
  });
});
